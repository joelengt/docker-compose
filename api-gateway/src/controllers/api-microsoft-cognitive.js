...
class MicrosoftCognitive {
  ...
  async appTest (req, res) {
    try {
      debug('STEP 3 >>>>')
      let service = await microsoftCognitiveService.app1()
      let payload = service
      debug('RESULT!!!  step 3>>>', payload)
      return res[`${payload.statusCode}`](payload.data, 'TEST success!')
    } catch (err) {
      debug('ERROR step 3', err)
      let payload = err
      return res[`${payload.statusCode}`](payload.data, 'TEST error!')
    }
  }
